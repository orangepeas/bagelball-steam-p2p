...
func add_player_steam(steam_id):
	...
	updateLobbyBoard()
	##multiplayer.get_peers()  doesn't work for some reason for the rpc_id loop
	for player in global.players.values():
		if player.lobbyHost == false:
			send_updated_players.rpc_id(player.multiplayer_id, global.players)
...
func _on_create_lobby_button_pressed() -> void:
	...
	Steam.createLobby(Steam.LobbyType.LOBBY_TYPE_PUBLIC, global.maxPlayers)
	#multiplayer.multiplayer_peer = peer
	$"../Create Lobby".hide()
	$"../PanelContainer".hide()
	$"../Lobby Menu V2".show()
